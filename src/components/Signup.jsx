...
const Signup = () => {
    ...
    const signUpsubmit = async (data) => {
        setError("");
        try {
            const { data: session, error: authError } = await supabase.signUp({ email: data.email, password: data.password, name: data.name });
    ...
    return (
        <Container className="flex flex-col w-full items-center">
            <div className=" max-w-lg w-full bg-white/0 backdrop-blur-[20px] shadow-2xl rounded-2xl p-7 sm:p-10 flex flex-col justify-center items-center gap-6">
                ...
                <div>
                    <h1>Already have an account? <span className='text-cyan-500 hover:underline'>  <Link to="/login">Sign Up
                    </Link></span></h1>
